...
let subtitleHistory = [];
let currentPartialSubtitle = null; 
let lastPartialText = '';

// 新的渲染状态：
let targetHistory = [];       // 保存翻译后的英文最终结果，最多3条
let currentSourceText = '';   // 当前正在识别的中文文本
let currentDisplay = 'en';    // 显示语言（本插件为 en）
...
function renderLine({ en, zh, isFinal, display }) {
  ...
  // zh-en插件: 历史显示英文翻译，当前显示中文识别
  const translatedText = en || '';  // 翻译后的英文
  const sourceText = zh || '';      // 正在识别的中文
  
  if (isFinal) {
    // 最终结果：只有英文翻译才加入历史
    if (translatedText && translatedText.trim() && translatedText !== sourceText) {
      // 简单检测是否为英文（包含字母且不全是中文）
      const hasEnglish = /[a-zA-Z]/.test(translatedText);
      const hasChinese = /[一-龥]/.test(translatedText);
      if (hasEnglish && !hasChinese && translatedText !== lastSubtitleText) {
        console.log('[Content] Adding to history (English):', translatedText);
...
function renderSeparatedSubtitles() {
  ...
  targetHistory.forEach((text, idx) => {
    ...
    if (!hasEnglish || hasChinese) {
      console.warn('[Content] Skipping non-English text in history:', text);
      return;
    }
